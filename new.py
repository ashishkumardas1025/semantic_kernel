...
def determine_query_type(query: str) -> str:
    ...
    # Check for intake-related keywords
    intake_keywords = ["intake", "onboarding", "registration", "enrollment", "application", "setup"]
    if any(keyword in query_lower for keyword in intake_keywords):
        return "intake"
    
    # Check for capability-related keywords
    capability_keywords = ["capability", "feature", "function", "service", "system", "module"]
    if any(keyword in query_lower for keyword in capability_keywords):
        return "capability"
    
    # Check for business description patterns
    business_keywords = ["business", "process", "workflow", "procedure", "operation", "description"]
    if any(keyword in query_lower for keyword in business_keywords):
        return "business_description"
    
    # Default to capability search
    return "capability"

def generate_response_with_links(query: str, matches: List[Dict[str, Any]], query_type: str) -> str:
    """Generate response based on query type using prompts"""
    if not matches:
    ...
    if query_type == "intake":
        system_prompt = """You are a business analyst focusing on intake and onboarding processes. 
        Provide a concise summary (under 200 words) of the top matching capability, focusing on:
        - How it relates to intake/onboarding processes
        - Key system changes required
        - Business value and impact
        Then mention that related source files are available for detailed review."""
        
        user_prompt = f"""
        Query: "{query}"
        
        Top matching capability:
        - Capability: {match_data[0]['capability']}
        - Business Description: {match_data[0]['business_description']}
        - System Changes: {match_data[0]['system_changes']}
        - Source: {match_data[0]['file_name']}
        
        Provide a focused summary and mention these source files contain similar capabilities:
        ...
        system_prompt = """You are a business analyst focusing on business processes and descriptions. 
        Provide a detailed summary (under 200 words) of the top matching capability, emphasizing:
        - Business process alignment
        - Implementation scope
        - Expected outcomes and value
        Then mention source files for detailed technical specifications."""
        
        user_prompt = f"""
        Query: "{query}"
        
        Top matching business process:
        - Capability: {match_data[0]['capability']}
        - Business Description: {match_data[0]['business_description']}
        - System Changes: {match_data[0]['system_changes']}
        
        Provide a business-focused summary and mention these source files for detailed specifications:
        {chr(10).join(source_links)}
        """
...
def search_capabilities(query: str, max_results: int = 5) -> Dict[str, Any]:
    """Main search function with intelligent query handling"""
    try:
        ...
        # Generate intelligent response with source links
        response = generate_response_with_links(query, similar_capabilities, query_type)
...
def main():
    ...
    for query in test_queries:
        ...
        print(f"Type: {result.get('query_type')}, Found: {result.get('total_found')}")
